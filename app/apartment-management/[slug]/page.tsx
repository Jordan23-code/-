...
import { getApartmentArticleBySlug } from "@/lib/articles";
...
export async function generateMetadata({
  ...
  const article = getApartmentArticleBySlug(params.slug);
  if (!article) return {};

  const avgMan = Math.round(article.stats.avgTradePrice / 10000).toLocaleString();
  const title = `${article.title} | アパート経営ラボ(仮)`;
  const description = `${article.prefecture}${article.city}のアパート経営・賃貸経営データを解説。${article.year}年の取引件数${article.stats.count}件・平均価格${avgMan}万円を、国土交通省の公式データと宅建士の実務目線で分析。`;
...
export default function ApartmentArticlePage({ params }: { params: { slug: string } }) {
  const article = getApartmentArticleBySlug(params.slug);
  ...
  return (
    <main className="min-h-screen bg-slate-50 py-10 px-4">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow border border-slate-200 p-8">
        <Link href="/apartment-management" className="text-sm text-slate-500 hover:underline">
          ← アパート経営ラボ一覧に戻る
        </Link>
        ...
        {article.sections.agentComment && (
          <div className="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
            <p className="text-xs font-bold text-indigo-700 mb-1">🏢 宅建士コメント</p>
        ...
        <section className="mb-6">
          <h2 className="font-bold text-lg mb-2">賃貸経営データ(国土交通省 不動産情報ライブラリより)</h2>
        ...
        {article.sections.propertyTrend && (
          <Section title="今、賃貸需要の観点でどんな物件が動いているか">{article.sections.propertyTrend}</Section>
        )}
        ...
        <Section title="アパート経営における表面利回り試算例">{article.sections.yieldSimulation}</Section>

        {article.sections.lowEntryPitch && (
          <section className="mb-6 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
            <h2 className="font-bold text-lg mb-2 text-indigo-800">複数戸所有で規模を広げたい方へ</h2>
            <p className="text-sm text-indigo-900 leading-relaxed whitespace-pre-line">
              {article.sections.lowEntryPitch}
            </p>
          </section>
        ...
        <div className="my-8 p-6 border border-dashed border-indigo-300 rounded-lg bg-indigo-50 text-center">
          <p className="text-sm font-bold text-indigo-800 mb-1">
            空室対策・賃貸管理の相談先を探してみませんか?
          </p>
          <p className="text-xs text-indigo-600 mb-3">
            賃貸管理会社やアパートローンの相談窓口を無料で比較できます
          </p>
        ...
        <div className="mt-6 p-4 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800 leading-relaxed">
          本記事は公開データに基づく一般的な情報提供を目的としており、特定の物件・投資判断を推奨するものではありません。実際の投資判断は、最新の情報を確認の上、宅地建物取引士等の専門家にご相談ください。
        </div>
      </div>
    </main>
  );
