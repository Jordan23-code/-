#!/usr/bin/env node
// アパート経営(賃貸経営視点)メディア: 国交省「不動産情報ライブラリ」の実データを取得し、
// AIで記事を自動生成して content/apartment-management/ に保存するスクリプト。
// (generate-article.mjs のアパート経営版。ロジックは共通、切り口とプロンプトのみ変更)
//
// 使い方:
//   node scripts/generate-apartment-article.mjs --pref 13 --city 13211 --cityName 小平市 --pref-name 東京都 --year 2024
//
...
async function fetchHeroImage(cityName) {
  ...
  const queries = [
    "japan apartment building exterior",
    "japan rental apartment building street",
    "tokyo suburb apartment complex",
    "japan low rise apartment building",
  ];
...
async function generateSections(stats, byType, yieldSim, districtRanking, ageBand, areaBand, priceTrend, lowEntry) {
  ...
  const prompt = `あなたは宅地建物取引士監修のもとで記事を書く、アパート経営・賃貸経営専門メディアのライターです。読者は「複数戸の賃貸物件(アパート・マンション一棟、または複数の区分所有)を保有・運営し、空室リスクを抑えながら安定した賃貸収益を目指す大家・賃貸経営者」です。個別の部屋を1つ買う視点ではなく、賃貸経営・大家業としての視点(空室対策、賃貸需要、複数戸所有によるリスク分散、管理運営)で記述してください。
...
{
  "title": "記事タイトル(このエリアのアパート経営・賃貸経営の視点がわかる、40文字程度。「大家」「賃貸経営」「アパート経営」等の語を含めてよい)",
  "overview": "上記の物件種別ごとの内訳を踏まえた相場の客観的な解説(300文字程度。種類を混同せず、種別ごとの違いに触れる。断定的な将来予測は避け、あくまで公式データの事実として記述する)",
  "propertyTrend": "人気地区ランキング・築年数帯・面積帯のデータを踏まえて「今このエリアでは賃貸需要の観点でどんな物件が多く取引されているか」を具体的に解説(350文字程度。地区名や築年数帯・面積帯に必ず言及すること。複数年の価格推移にも触れ、上昇/下落の事実を淡々と述べる。空室リスクや賃貸需要の観点も交える。将来予測や断定的な投資判断は避ける)",
  "yieldSimulation": "上記の利回り試算の文章を使い、前後に解説を加えたもの(数字は書き換えないこと。アパート経営における表面利回りの一例であることを明記する。250文字程度)",
  "lowEntryPitch": "総額を抑えて始めやすい区分のデータを使い、「複数戸所有で規模を広げたい大家・賃貸経営者」に向けた解説(200文字程度。利回りが高いとは書かず、あくまで総額が低く複数戸への分散投資がしやすい点を根拠にすること)",
  "agentComment": "宅地建物取引士としての実務コメント(200〜250文字程度)。前半はこのエリアのデータの要点を断定調で述べ(「〜です」「〜となっています」。「〜と感じています」「〜と確認しています」の繰り返しは禁止)、アパート経営・賃貸経営を検討する方への具体的な提案や一言(空室対策・管理の観点など)を加えること。特定物件の推奨や将来の価格保証はしない。後半に次の2文を必ずそのまま追加すること:『表面利回りの試算では概ね6%前後が目安ですが、これは一律の想定家賃に基づく参考値であり、エリア間の優劣を示すものではありません。また、アパートローンは住宅ローンとは異なり事業性融資として扱われ、金利や融資期間の条件が金融機関ごとに異なる点にご注意ください。』",
  "beginnerTips": "このエリアでアパート経営・賃貸経営を検討する初心者向けの一般的な注意点(具体的な断定助言ではなく、確認すべき観点を提示する形。空室リスク・賃貸需要の見極め・管理委託・原状回復費用など賃貸経営特有の観点を含める。250文字程度)"
}`;
...
function saveArticle(article) {
  const dir = path.join(ROOT, "content", "apartment-management");
