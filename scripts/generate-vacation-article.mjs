#!/usr/bin/env node
// 別荘・空き家リノベメディア: 国交省「不動産情報ライブラリ」の実データを取得し、
// AIで記事を自動生成して content/vacation-homes/ に保存するスクリプト。
// (generate-article.mjs の別荘・空き家版。ロジックは共通、切り口とプロンプトのみ変更)
//
// 使い方:
//   node scripts/generate-vacation-article.mjs --pref 14 --city 14210 --cityName 三浦市 --pref-name 神奈川県 --year 2024
//
...
async function fetchHeroImage(cityName) {
  ...
  const queries = [
    "japan old countryside house",
    "japan akiya abandoned house exterior",
    "japan mountain village old wooden house",
    "japan coastal countryside cottage",
  ];
...
async function generateSections(stats, byType, yieldSim, districtRanking, ageBand, areaBand, priceTrend, lowEntry) {
  ...
  const prompt = `あなたは宅地建物取引士監修のもとで記事を書く、別荘・空き家リノベーション専門メディアのライターです。読者は「関東近郊の総額を抑えられる古い別荘・空き家を購入し、セカンドハウスやDIYリノベーションとして活用したい」層です。あくまで市場データの解説であり、個別物件のリノベーション費用や施工可否を保証する記事ではありません。
...
{
  "title": "記事タイトル(このエリアの空き家・別荘相場がわかる、40文字程度。「格安」「セカンドハウス」等の語を含めてよい)",
  "overview": "上記の物件種別ごとの内訳を踏まえた相場の客観的な解説(300文字程度。種類を混同せず、種別ごとの違いに触れる。断定的な将来予測は避け、あくまで公式データの事実として記述する)",
  "propertyTrend": "人気地区ランキング・築年数帯・面積帯のデータを踏まえて「今このエリアではどんな空き家・別荘が多く取引されているか」を具体的に解説(350文字程度。地区名や築年数帯・面積帯に必ず言及すること。複数年の価格推移にも触れ、上昇/下落の事実を淡々と述べる。将来予測や断定的な判断は避ける)",
  "yieldSimulation": "上記の利回り試算の文章を使い、前後に解説を加えたもの(数字は書き換えないこと。あくまで賃貸に出す場合の一例であることを明記する。250文字程度)",
  "lowEntryPitch": "総額を抑えて始めやすい区分のデータを使い、「格安の古い別荘・空き家をセカンドハウスやDIYリノベーションとして活用したい層」に向けた解説(200文字程度。リノベ費用や仕上がりを保証する表現は使わず、あくまで購入時の総額が低い点を根拠にすること)",
  "agentComment": "宅地建物取引士としての実務コメント(200〜250文字程度)。前半はこのエリアのデータの要点を断定調で述べ(「〜です」「〜となっています」。「〜と感じています」「〜と確認しています」の繰り返しは禁止)、別荘・空き家を検討する方への具体的な提案や一言を加えること。特定物件の推奨や将来の価格保証、リノベーション後の価値保証はしない。後半に次の2文を必ずそのまま追加すること:『表面利回りの試算では概ね6%前後が目安ですが、これは一律の想定家賃に基づく参考値であり、エリア間の優劣を示すものではありません。また、別荘・セカンドハウス向けの物件は住宅ローンの対象外となる金融機関が多く、購入方法の確認が必要な点にもご注意ください。』",
  "beginnerTips": "このエリアで空き家・別荘を検討する初心者向けの一般的な注意点(具体的な断定助言ではなく、確認すべき観点を提示する形。積雪・凍結・水道管理・アクセス・自治体の空き家バンク制度など別荘特有の観点を含める。250文字程度)"
}`;
...
function saveArticle(article) {
  const dir = path.join(ROOT, "content", "vacation-homes");
